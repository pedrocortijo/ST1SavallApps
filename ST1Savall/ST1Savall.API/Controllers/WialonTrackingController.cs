        ...
        foreach (var camion in camiones)
        {
            ...
            try
            {
                ...
                var ubicacion = new UbicacionCamionWialon
                {
                    ...
                    DistanciaRutaMetros = servicioAsignado is null ? null : servicioFirmado ? (servicioAsignado.DistanciaObraDescargaMetros ?? 0) + (servicioAsignado.DistanciaDescargaRegresoMetros ?? 0) : recorridoCompleto ? servicioAsignado.DistanciaTotalMetros : servicioAsignado.DistanciaOrigenObraMetros,
                    MinutosViajeRuta = servicioAsignado is null ? null : servicioFirmado ? (servicioAsignado.MinutosObraDescarga ?? 0) + (servicioAsignado.MinutosDescargaRegreso ?? 0) : recorridoCompleto ? servicioAsignado.DuracionViajeMinutos : servicioAsignado.MinutosOrigenObra,
